...
export const description =
  'Verify a UI change you just made matches what you intended, before reporting the task complete. Use this whenever you modify a route, page, or component and need to confirm the visual result.\n\nReturns a verdict (`pass`, `expected_change_detected`, `unexpected_regression`, `no_change_detected`, or `needs_human_review`) and a `next_action` telling you whether to proceed, request human review, rollback, or re-verify your deployment. Do not mark the task done unless the verdict is `pass` or `expected_change_detected` with explicit human approval.\n\nRequires a SnapDiff project + page baseline. Pass `intent` to describe in one sentence what you meant to change. For sharper verdicts, pass `intent_regions` with either a CSS `selector` (recommended for code-editing agents — SnapDiff resolves it to a bbox during capture) or a `bbox` directly (when your agent drives a browser and already knows pixel coordinates).';
